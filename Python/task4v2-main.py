...
#################################################################
'''
brute forcing all the possibility with min threshold to speed up little bir
'''
########################################################
...
for i in extra:
    x = iter(permutations(i))
    for j in x:
        bigdat.append(list(j));
...
ld = []
for o,(i,y) in enumerate(bigdat[0]):
    ld.append(y)
    if o != len(extra[0])-1:
        ld.append(',')
        ld.append(' ')
...
for o,(i,y) in enumerate(bigdat[0]):    
    ...
    if o != len(extra[0])-1:    
        ld.append(',')
        ld.append(' ')
mm  =  parsev2.finalizer(''.join(ld)+'#',99999999)[0]
...
for z,k in enumerate(bigdat):
    ...
    for o,(i,y) in enumerate(k):    
        ...
        if o != len(extra[0])-1:    
            ld.append(',')
            ld.append(' ')    
    dtemp = parsev2.finalizer(''.join(ld)+'#',mm)
    if dtemp[0] <= mm:
            mm = dtemp[0]
            resource.append((dtemp,z))
...
ld = []
for o,(i,y) in enumerate(bigdat[result]):
    ld.append(y)
    if o != len(extra[0])-1:
        ld.append(',')
        ld.append(' ')
...
print(''.join(ld))
#print('14, 19, 14, 12, 7, 12, 9, 12\nA1, F1, F5, E6, A5, A6, B6, E1')
